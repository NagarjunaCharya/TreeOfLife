"""
Fine-tune DNABERT-2 - EPOCH 5 (FINAL) + TEST EVALUATION
Loads checkpoint from epoch 4
"""
...
def main():
    ...
    print('=' * 70)
    print('  DNABERT-2 Fine-Tuning - EPOCH 5/5 (FINAL)')
    print('=' * 70)
    ...
    # Load tokenizer
    print('\n[1/5] Loading DNABERT-2 tokenizer...')
    ...
    print('\n  --- Test Set ---')
    test_dataset = CropDNADataset('test', DATASET_DIR, tokenizer, max_length=max_length, max_samples_per_crop=eval_samples)
    ...
    print('\n[3/5] Loading model from epoch 4 checkpoint...')
    model = AutoModelForSequenceClassification.from_pretrained(OUTPUT_DIR, num_labels=NUM_LABELS, trust_remote_code=True)
    model.to(device)

    checkpoint_path = os.path.join(OUTPUT_DIR, 'checkpoint_epoch4.pt')
    if os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location=device)
        model.load_state_dict(checkpoint['model_state'])
        print('  ✓ Loaded model state from epoch 4')
    ...
    # Save final model
    print(f'    Saving final model after epoch 5...')
    ...
    torch.save(checkpoint, os.path.join(OUTPUT_DIR, 'checkpoint_epoch5.pt'))

    # Final evaluation on test set
    print(f'\n[5/5] Evaluating final model on test set...')
    ...
    target_names = [CROP_LABELS[i] for i in range(NUM_LABELS)]
    ...
    print(f'  {"":>10s}', '  '.join(f'{n:>7s}' for n in target_names))
    for i, row in enumerate(cm):
        print(f'  {target_names[i]:>10s}', '  '.join(f'{v:>7d}' for v in row))

    # Save test results
    test_results = {
        'accuracy': round(test_metrics['accuracy'], 2),
        'f1_macro': round(test_metrics['f1_macro'], 2),
        'f1_weighted': round(test_metrics['f1_weighted'], 2),
