"""
Fine-tune DNABERT-2 - EPOCH 1 ONLY
GPU-optimized for RTX 4050 (6 GB VRAM)
"""
...
def main():
    ...
    print('=' * 70)
    print('  DNABERT-2 Fine-Tuning - EPOCH 1/5')
    print('=' * 70)
    ...
    # Load model
    print('\n[3/4] Loading DNABERT-2 model...')
    model = AutoModelForSequenceClassification.from_pretrained(MODEL_PATH, num_labels=NUM_LABELS, trust_remote_code=True)
    model.to(device)
    ...
    if torch.cuda.is_available():
        ...
        print(f'  GPU memory used (model): {mem_used:.0f} MB')
    ...
    # Save model after epoch 1
    print(f'    Saving model after epoch 1...')
    ...
    torch.save(checkpoint, os.path.join(OUTPUT_DIR, 'checkpoint_epoch1.pt'))

    print(f'\n{"=" * 70}')
    print(f'  Epoch 1 Complete! Model and checkpoint saved.')
    print(f'  Next: Run train_epoch_2.py')
    print(f'{"=" * 70}')
