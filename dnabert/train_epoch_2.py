"""
Fine-tune DNABERT-2 - EPOCH 2 ONLY
...
def main():
    ...
    print('=' * 70)
    print('  DNABERT-2 Fine-Tuning - EPOCH 2/5')
    print('=' * 70)
    ...
    print('\n[3/4] Loading model from epoch 1 checkpoint...')
    model = AutoModelForSequenceClassification.from_pretrained(OUTPUT_DIR, num_labels=NUM_LABELS, trust_remote_code=True)
    model.to(device)

    checkpoint_path = os.path.join(OUTPUT_DIR, 'checkpoint_epoch1.pt')
    if os.path.exists(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location=device)
        model.load_state_dict(checkpoint['model_state'])
        print('  ✓ Loaded model state from epoch 1')
    ...
    # Save model after epoch 2
    print(f'    Saving model after epoch 2...')
    ...
    torch.save(checkpoint, os.path.join(OUTPUT_DIR, 'checkpoint_epoch2.pt'))

    print(f'\n{"=" * 70}')
    print(f'  Epoch 2 Complete! Model and checkpoint saved.')
    print(f'  Next: Run train_epoch_3.py')
    print(f'{"=" * 70}')
